...
async function saveImage(image, dirPath) {
    try {
        if (!image) {
            logger.error("Echec de la sauvegarde de l'image. L'image n'existe pas.");
            return null;
        }
        ...
        if (!base64Prefix.test(image)) {
            logger.error("Format de l'image invalide. Attendu: data:image/png;base64");
            return null;
        }
        ...
        logger.info(`Photo de profil de l'utilisateur sauvegardée avec succès à l'emplacement: ${fullPath}`);
        return relativePath;
    } catch (error) {
