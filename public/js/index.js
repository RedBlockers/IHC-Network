marked.use({
  ...
  document.getElementById('messageInput').addEventListener('keyup', function(event) {
    ...
    if (this.scrollHeight > maxHeight) {
        ...
        const newHeight = window.innerHeight * 0.7 - this.scrollHeight + 12; // 20 pour le padding et autres marges
    ...
    messageElement.innerHTML = `
      <div class="message-actions bg-dark text-white position-relative">
        <div class="message-content bg-dark text-white">
          ${marked.parse(message.content.replace(/\n/g, '<br>\n\n'))}
        </div>
  ...
  function sendMessage(){
    ...
    if (messageContent.trim() !== '') {
      axios.post('/messages', { content: messageContent }).then(() => {
